...
import {
  ...
  TitleCustomer,
  TotalsCard,
} from "./features/measure/components/MeasureUi";
import {
  calcAccessoriesLines,
  computeLine,
  computeTotals,
  headrailSurcharge,
  isControlNeedsSideLen,
  isMotorAllowedByHeadrail,
  lChannelPrice,
  lenValue,
  lrValue,
  normalizeBottomBy,
  ...
  resolveItem,
  sideChannelPrice,
} from "./features/measure/measureLogic";
  ...
  return (
    <div className="min-h-screen">
      <main className="max-w-6xl mx-auto px-4 py-3">
        ...
        {(role==="admin" || role==="worker") && (
          <div className={tab==="Office" ? "block" : "hidden"}>
            <OrdersPage
              toast={toast}
              SUPA_ON={SUPA_ON}
              supabase={supabase}
              supaFetchServerJobs={supaFetchServerJobs}
              supaDeleteServerJob={supaDeleteServerJob}
              computeTotals={computeTotals}
              computeLine={computeLine}
              normalizeItem={normalizeItem}
              TitleCustomer={TitleCustomer}
              ...
              AccessoriesSummaryCard={AccessoriesSummaryCard}
              TotalsCard={TotalsCard}
              HW_COLOR_LABELS={HW_COLOR_LABELS}
              in2={in2}
              splitFtIn32={splitFtIn32}
              inToMm={inToMm}
              fracLabel={fracLabel}
              round2={round2}
              XLS_WITH_SQFT_SUM={XLS_WITH_SQFT_SUM}
              appendSqftTotalRow={appendSqftTotalRow}
              buildSplitSummary={buildSplitSummary}
              calcAccessoriesLines={calcAccessoriesLines}
              XL_STYLE={XL_STYLE}
              sectionTitleHTML={sectionTitleHTML}
              makeOptionsLabelRow={makeOptionsLabelRow}
              esc={esc}
        ...
        {role==="admin" && (
          <div className={tab==="Admin"  ? "block" : "hidden"}>
            <AdminPage
              SUPA_ON={SUPA_ON}
              supaFetchTemplates={supaFetchTemplates}
              supaUpsertTemplate={supaUpsertTemplate}
              supaDeleteTemplate={supaDeleteTemplate}
            />
          </div>
        )}
      </main>
