  ...
  return (
    <div className="min-h-screen">
      <main className="max-w-6xl mx-auto px-4 py-3">
        ...
        <div className={tab==="Drafts"  ? "block" : "hidden"}><DraftsLocal toast={toast} /></div>
        {(role==="admin" || role==="worker") && (
...
function DraftsLocal({ toast }){
  ...
  async function sendToOffice(job){
    if(!SUPA_ON){ toast.err("Failed to send. Check connection/keys."); return; }
    if(typeof navigator!=="undefined" && "onLine" in navigator && !navigator.onLine){
      toast.err("Internet required to send."); return;
    }
    try{
      await supaInsertJob(job);
  ...
  <table className="min-w-full text-sm">
    ...
    <tbody>
      {(job.items||[]).map((it,i)=>{
        const excluded = it?.include === false;     // ★ 추가: 제외 플래그
        const c = computeLine(it);
        const fabricNo = canonicalFabricNo(it);

        return (
           <tr key={i}
              className={`border-b ${excluded ? "bg-gray-100 text-gray-400" : ""}`}>  {/* ★ 제외행 회색 처리 */}
            <td className="p-2">{i+1}</td>
